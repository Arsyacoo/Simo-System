...
export default function Login() {
  ...
  const demoAccounts = [
    { name: 'Rina Wijaya', role: 'Owner', email: 'rina.wijaya@simo.test' },
    { name: 'Budi Santoso', role: 'Production Manager', email: 'budi.santoso@simo.test' },
    { name: 'Joko Anwar', role: 'Foreman', email: 'joko.anwar@simo.test' },
    { name: 'Siti Nurhaliza', role: 'QC Inspector', email: 'siti.nurhaliza@simo.test' },
    { name: 'Dewi Lestari', role: 'Admin', email: 'dewi.lestari@simo.test' },
  ];
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-4 sm:p-6 font-sans">
      <div className="w-full max-w-5xl grid md:grid-cols-12 gap-8 items-center">
        ...
        <div className="md:col-span-5 text-white space-y-6 hidden md:block">
          ...
          <div className="space-y-4 pt-4 border-t border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-medium text-slate-300">Resilient Offline-First Sync</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="h-2 w-2 rounded-full bg-indigo-500"></div>
              <span className="text-sm font-medium text-slate-300">Role-Based Access Security</span>
            </div>
            <div className="flex items-center gap-3">
              <div className="h-2 w-2 rounded-full bg-emerald-500"></div>
              <span className="text-sm font-medium text-slate-300">Cloud PostgreSQL Powered</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-7 bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl shadow-black/40">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Masuk ke Sistem</h2>
          <p className="text-slate-400 text-sm mb-6">Gunakan akun demo di bawah untuk login cepat.</p>
              ...
              {demoAccounts.map((account) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-between text-left p-3 rounded-xl border transition-all text-xs ${
                      isSelected
                  ...
                  >
                    <div>
                      <span className="block font-bold text-white leading-tight">{account.name}</span>
                      <span className="block text-slate-400 mt-0.5">{account.role}</span>
                    </div>
